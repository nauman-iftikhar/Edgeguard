...
# ══════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════
# TELEGRAM CLUSTER HEALTH MONITOR
# ══════════════════════════════════════════════════════════════
import threading, subprocess, requests as req
...
# State tracking — only alert on changes
_node_state = {}
_pod_state  = {}
...
def check_nodes():
    print(f"[HealthMonitor] Checking nodes...")
    try:
        r = req.get(
        ...
        online = {res["metric"]["instance"].split(":")[0]: res["value"][1] == "1"
                  for res in results}
        ...
        print(f"[HealthMonitor] Node check error: {e}")
...
def check_pods():
    try:
        ...
        print(f"[HealthMonitor] Pod check error: {e}")

def check_temperature():
    try:
        r = req.get(
            f"{PROMETHEUS_URL}/api/v1/query",
            params={"query": "node_thermal_zone_temp > 75000"},
            timeout=5
        )
        results = r.json()["data"]["result"]
        for res in results:
            ...
            tg_send(
                ...
                f"🖥️ <b>Node:</b> {node}\n"
                f"🌡️ <b>Temperature:</b> {temp:.1f}°C\n"
                f"⚠️ Risk of CPU throttling above 80°C"
            )
    except Exception as e:
        print(f"[HealthMonitor] Temperature check error: {e}")

def health_monitor_loop():
    print("[HealthMonitor] Started — checking every 30s")
    while True:
        try:
            check_nodes()
            check_pods()
...
if __name__ == "__main__":
    ...
    print("[HealthMonitor] Cluster health monitoring started")
