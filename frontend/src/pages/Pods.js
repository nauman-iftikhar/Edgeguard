...
export default function Pods() {
  ...
  const drawLines = () => {
    ...
    const newLines = []
    pods.forEach(pod => {
      // Use pod.node directly — matches node card IDs
      const nodeEl = document.getElementById(`node-${pod.node}`)
  ...
  // Build pod map — use pod.node directly (matches node names from API)
  const podsByNode = {}
  pods.forEach(pod => {
    if (!podsByNode[pod.node]) podsByNode[pod.node] = []
    podsByNode[pod.node].push(pod.type)
  })
          ...
          {pi4Node && !pi4Active && (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: 64, lineHeight: 1, filter: 'drop-shadow(0 0 8px #378ADD)' }}>🚑</div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 20 }}>
                    ...
                    <div>
                      ...
                      <p style={{ color: '#EF9F27', fontSize: 18, fontWeight: 'bold', margin: 0 }}>
                        {Math.floor((pi4Node.uptime || 0) / 3600)}h
                      </p>
                    </div>
                    <div>
